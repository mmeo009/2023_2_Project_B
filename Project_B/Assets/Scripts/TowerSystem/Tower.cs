...
public class Tower : MonoBehaviour
{
    ...
    public float checkTime = 0.2f;      // 0.2초 마다 검출

    public bool enemiesUpdate;          // flag 값으로 체크 완료했는지 검출
    void Start()
    {
        checkCounter = checkTime;       // 지정한 시간을 CheckCounte 에 입력
    }
    ...
    void Update()
    {
        ...
        if(checkCounter <= 0)           // 0초 이하가 되었을때
        {
            ...
            enemiesinRange.Clear();     // 리스트 초기화(기존에 사용했을수도 있기 때문)
