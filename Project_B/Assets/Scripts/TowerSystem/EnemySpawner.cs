...
public class EnemySpawner : MonoBehaviour
{
    ...
    public float spawnCounter;              // 시간을 흐르게 하여 스폰되는 시간 설정
    ...
    void Update()
    {
        ...
        {
            spawnCounter -= Time.deltaTime; // timeBetweenSpawns에서 설정한 시간에서 -> 0초로 간다
            if(spawnCounter <= 0 )
            {
                spawnCounter = timeBetweenSpawns;   // 0초 이하인 카운터에 다시 설정한 시간을 입력한다
                ...
                amountToSpawn -= 1;         // 소환한 후에 숫자를 1빼준다
            }
        }
    }
}
